...
Validar el correcto funcionamiento del módulo Candy Store en la aplicación móvil, garantizando que los usuarios puedan explorar el catálogo de productos de confitería filtrados por sucursal y categoría mediante gestos nativos, gestionar el carrito de compras local de manera fluida y avanzar correctamente hacia el flujo de pago o compra mixta.

Las pruebas contemplan la sincronización de la disponibilidad, las restricciones de sesión protegidas por la app, el manejo del ciclo de vida (segundo plano) y la adaptabilidad de la interfaz en diversas densidades de pantalla y orientaciones móviles.
...
- Consulta y renderizado del catálogo móvil de confitería.
- Selección y cambio de sucursal mediante selectores o modales nativos.
- Filtrado rápido por categoría utilizando barras de desplazamiento horizontal (Carousel / Tabs).
- Gestión del carrito desde el dispositivo móvil (gestos para incrementar, disminuir o eliminar).
- Restricciones de autenticación integradas con el flujo de la app.
- Interrupciones de la app y estados del ciclo de vida mientras se gestiona el carrito.
- Diseño responsivo y guías de interfaz móvil (Android).
- Integración con Backend y manejo de fallos de red celular.
...
TC-CAN-088 – Orden del foco.
TC-CAN-089 – Contraste adecuado.
TC-CAN-090 – Lectura mediante lector de pantalla.
TC-CAN-091 – Texto alternativo en imágenes.
TC-CAN-092 – Etiquetas accesibles para botones.
TC-CAN-093 – Tamaño adecuado de botones táctiles.

## Responsive Testing
TC-CAN-094 – Visualización Tablet.
TC-CAN-095 – Visualización Mobile.
TC-CAN-096 – Cambio entre orientación vertical y horizontal.
TC-CAN-097 – Adaptación de tarjetas de productos.
TC-CAN-098 – Adaptación del selector de sucursales.
TC-CAN-099 – Adaptación del filtro por categorías.
TC-CAN-100 –  Verificar ausencia de scroll horizontal.
TC-CAN-101 – Correcta visualización del carrito.

## UI / UX Testing
TC-CAN-102 – Consistencia de colores institucionales.
TC-CAN-103 – Consistencia tipográfica.
TC-CAN-104 – Correcta alineación de tarjetas.
TC-CAN-105 – Espaciado uniforme entre productos.
TC-CAN-106 – Indicadores visuales al agregar productos.
TC-CAN-107 – Feedback visual durante la carga del catálogo.
TC-CAN-108 – Indicador visual para productos agotados.
TC-CAN-109 – Claridad de los mensajes mostrados al usuario.
TC-CAN-110 – Fluidez del recorrido hacia el proceso de compra.